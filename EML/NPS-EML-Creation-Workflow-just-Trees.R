...
# NPS-EML-Creation-Workflow-just-Trees.R
...
data_files <- c("PlotCoordinatesDD.csv",
                "TreeTally.csv")
...
data_names <- c("Sampling Plot Coordinates",
...
data_descriptions <- c("Sampling Plot Coordinates in latitude - longitude decimal degrees",
...
data_taxa_tables <- c("TreeSpeciesNames.csv")
...
# refer to https://ediorg.github.io/EMLassemblyline/articles/edit_tmplts.html for helpful hints 
# and `view_unit_dictionary()` for potential units. This will only need to be run again if the 
# attributes (name, order or new/deleted fields) are modified from the previous year. NOTE that if 
