...
# Unique list - PASS
# CanopyHeight frequency histogram - PASS
...
canopyheight |>
  filter(CanopyHeight < 10) |>
  arrange(CanopyHeight) |>
  print(n = 604) 
...
# canopyheight frequencies - PASS

view(basalarea)

basalarea |> filter(canopyheight == 0) |>
   print(n = 350)

ggplot(basalarea, aes(x = canopyheight)) + 
  geom_bar()

basalarea |> count (canopyheight) |> arrange (canopyheight, (n))
